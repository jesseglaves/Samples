<?
	...
	$path = explode('/', $uri);
	$route = end($path);
	$category = $eventsMod->getCategoryByRoute($route);
	$events = $eventsMod->getUpcomingEventsInCategories($limit = 20,$category,false);
	$cats = array_values($category);
	$page_title = "Events by Category";
...
<div class="row outercontent">
        ...
        <div class="events desktop-8 tablet-4 mobile-full padded">
        	...
        	<h2 class="roller-heading" style="border:none;margin-bottom:0 !important;padding:0;line-height:normal;height:inherit;text-align:left;"><?=$cats[2]?> Events</h2>
